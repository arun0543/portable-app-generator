...
if [[ -z "${_PAG_LIB_APPLICATION_SH:-}" ]]; then
	...
	application_create() {
		...
		# Generate a default entrypoint for independent execution
		local run_script="${instance_dir}/run.sh"
		if [[ ! -f "${run_script}" ]]; then
			cat > "${run_script}" << 'EOF'
#!/usr/bin/env bash
set -euo pipefail

# Default PAG Entrypoint
echo "Starting application..."
echo "To implement your application logic, edit this file or the scripts in app/"
EOF
			chmod +x "${run_script}"
		fi
