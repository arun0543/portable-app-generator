...
if [[ -z "${_PAG_LIB_CLI_SH:-}" ]]; then
	...
	cli_main() {
		...
		if cli_is_command_registered "${cmd}"; then
			...
			# Provide suggestions
			local avail_cmds
			avail_cmds="$(cli_get_registered_commands)"
			local suggestion=""
			local c
			for c in ${avail_cmds}; do
				if [[ "${c}" == "${cmd}"* ]]; then
					suggestion="${c}"
					break
				fi
			...
			if [[ -n "${suggestion}" ]]; then
				log_info "Did you mean 'pag ${suggestion}'?"
			else
				log_info "Run 'pag help' for a list of available commands."
			fi
